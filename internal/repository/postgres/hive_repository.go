...
func (r *HiveRepository) Create(ctx context.Context, h *hive.Hive) error {
	const q = `
		INSERT INTO hives (id, apiary_id, user_id, name, location, notes, created_at, updated_at)
		VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
	`

	_, err := r.db.Exec(ctx, q, h.ID, h.ApiaryID, h.UserID, h.Name, h.Location, h.Notes, h.CreatedAt, h.UpdatedAt)
	if err != nil {
...
func (r *HiveRepository) GetByID(ctx context.Context, userID, hiveID uuid.UUID) (*hive.Hive, error) {
	...
	err := r.db.QueryRow(ctx, q, hiveID, userID).Scan(
		&h.ID, &h.ApiaryID, &h.UserID, &h.Name, &h.Location, &h.Notes, &h.CreatedAt, &h.UpdatedAt, &h.DeletedAt,
	)
	if err != nil {
...
func (r *HiveRepository) Update(ctx context.Context, h *hive.Hive) error {
	...
	tag, err := r.db.Exec(ctx, q, h.Name, h.Location, h.Notes, h.UpdatedAt, h.ID, h.UserID)
	if err != nil {
